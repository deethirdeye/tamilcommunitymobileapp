...
export const useDescriptionAndRecordingHandlersSomeone = (state: StateSomeone) => {
  ...
  const uploadRecording = async () => {
    ...
    try {
      ...
      formData.append('audioFile', {
        uri: fileUri,
        name: `recording_${Date.now()}.m4a`, // Change file name to mp3
        type: 'audio/m4a', // Update MIME type for mp3
      } as any);
